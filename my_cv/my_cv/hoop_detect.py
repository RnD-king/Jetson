...
class HoopDetectorNode(Node):
    ...
    def make_band_masks(self, src_pts, width, height, t, s, roi_shape):
        ...
        def warp_poly(dst_poly):
            pts = cv2.perspectiveTransform(dst_poly.reshape(-1,1,2), M_inv).reshape(-1,2)
            return np.round(pts).astype(np.int32)

        top_src   = warp_poly(top_dst)
        left_src  = warp_poly(left_dst)
        right_src = warp_poly(right_dst)
        inner_src = warp_poly(inner_dst)
    ...
    def image_callback(self, color_msg: Image, depth_msg: Image):
        ...
        for cnt in contours:
            ...
            if area > self.backboard_area: # 1. 일정 넓이 이상
                ...
                band_mask, inner_mask = self.make_band_masks(src, width, height, t, s, roi_color.shape)

                area_band = cv2.countNonZero(band_mask)
                if area_band == 0:
                    continue

                # 빨강 비율
                red_hits = cv2.countNonZero(cv2.bitwise_and(red_mask, band_mask))
                ratio_band = red_hits / float(area_band)

                if ratio_band >= self.red_ratio_min:
                    # 내부 하양 비율
        ...
        if best_cx is not None:
            # 좌우 깊이로 yaw 추정
            # delta_px = max(5, int(0.15 * best_w))
            # point_left = best_cx - delta_px
            # point_right = best_cx + delta_px
            # point_y = max(0, min(self.image_height - 1, best_cy))

            # pixel = 3

            # # left_depth_vals = depth[point_y - pixel:point_y + pixel+1, point_left - pixel:point_left + pixel+1]
            # # right_depth_vals = depth[point_y - pixel:point_y + pixel+1, point_right - pixel:point_right + pixel+1]
            # # left_valid = np.isfinite(left_depth_vals) & (left_depth_vals > self.depth_min) & (left_depth_vals < self.depth_max)
            # # right_valid = np.isfinite(right_depth_vals) & (right_depth_vals > self.depth_min) & (right_depth_vals < self.depth_max)
            ...
            # for j in range(-6, 6 + 1):     # y 오프셋: -pixel..+pixel
            #     y = point_y + j
            #     for i in range(-pixel, pixel + 1): # x 오프셋: -pixel..+pixel
            #         # 왼쪽 창
            #         dL = depth[y, point_left + i]
            #         if (self.depth_min < dL < self.depth_max):
            #             z_left_sum += float(dL)
            #             cnt_left += 1
            #             cv2.circle(frame, (point_left + i, y), 1, (0, 0, 0), -1)

            #         # 오른쪽 창
            #         dR = depth[y, point_right + i]
            #         if (self.depth_min < dR < self.depth_max):
            #             z_right_sum += float(dR)
            #             cnt_right += 1
            #             cv2.circle(frame, (point_right + i, y), 1, (0, 0, 0), -1)
                    
            # yaw_deg = None
            # if cnt_left > 0 and cnt_right > 0:
            #     z_left_mean = (z_left_sum / cnt_left) * self.depth_scale  # mm→m
            #     z_right_mean = (z_right_sum / cnt_right) * self.depth_scale # mm→m

            #     x_left = (float(point_left) - self.cx_intr) * z_left_mean / self.fx
            #     x_right = (float(point_right) - self.cx_intr) * z_right_mean / self.fx

            #     dx = x_right - x_left
            #     dz = z_left_mean - z_right_mean  # 오른쪽이 더 가깝다면 +

            #     if abs(dx) > 1e-3:  # 1mm 이하면 불안정
            #         yaw_rad = math.atan2(dz, dx)
            #         yaw_deg = round(math.degrees(yaw_rad), 2)

            #         # 표시 점 (화면 기준 좌/우에 그대로 찍기)
                    

            #     self.get_logger().info(
            #         f"mean ZL={z_left_mean:.3f}m, ZR={z_right_mean:.3f}m (cntL={cnt_left}, cntR={cnt_right}), yaw={yaw_deg}"
            #     )
            # else:
            #     self.get_logger().info(f"not enough valid depth (cntL={cnt_left}, cntR={cnt_right})")
