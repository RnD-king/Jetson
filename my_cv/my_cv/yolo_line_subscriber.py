...
class LineListenerNode(Node):
    def __init__(self):
        ...
        # 파라미터 선언
        self.declare_parameter("limit_x", 40) # 핑크 라인
        self.declare_parameter("max_len", 30) # 최빈값
        self.declare_parameter("delta_s", 15) # 커브 판단할 때
        self.declare_parameter("vertical", 75) # 직진 판단 각도
        self.declare_parameter("horizonal", 15) # 수평 판단 각도
    ...
    def color_image_callback(self, msg): # 이미지 받아오기
        ...
        screen_h, screen_w = cv_image.shape[:2]  # 480 / 640
...
#---------------------------------------------------------------판단 시작  
        ...
        if self.candidates:
            ...
            if avg_x - zandi_x > self.limit_x:
                self.out_text = "Out Left"
                # angle -= (avg_x - zandi_x) / 2   <<<<  얼마나 벗어났느냐에 따라 목표 회전값 보정
            elif avg_x - zandi_x <  -self.limit_x:
                self.out_text = "Out Right"
                # angle += (avg_x - zandi_x) / 2
            else:
                self.out_text = "Straight"
        
        # ───────── 2. 방향 판단 ─────────
        if len(self.candidates) >= 3:  # 1. 점 세 개 이상 탐지
            ...
            if float(xs.max() - xs.min()) < 2.0: # 1) 거의 수직
                ...
                if delta >  self.delta_s: 
                    ...
                    tip_angle = math.degrees(math.atan(-(tip_y - zandi_y) / (tip_x - zandi_x)))
        ...
        elif len(self.candidates) == 2:  # 2. 점 두 개 탐지
            # 단순 선
        ...
        cv2.line(cv_image, (int(screen_w / 2) + self.limit_x, roi_y_start), (int(screen_w / 2) + self.limit_x, roi_y_end), (255, 22, 255), 1)
        cv2.circle(cv_image, (zandi_x, zandi_y), 5, (255, 255, 255), -1)
        #-------------------------------------------------------------------------------------------------- 프레임 처리 시간 측정
