...
class LineListenerNode(Node):
    ...
    def motion_callback(self, msg: MotionEnd): # 모션 끝났다 신호 받으면
        self.motion_end = bool(msg.motion_end_detect)
        if self.motion_end and not self.collecting:
        ...
        if self.collecting: # 모션이 끝났다고 신호 받으면 시작
            ...
            if len(self.candidates) >= 3:  #========================================================================================= I. 세 점 이상 탐지
                ...
                if float(line_x.max() - line_x.min()) < 2.0:  # 일직선인 경우
                    ...
                    line_angle = math.degrees(math.atan2(vx, vy))  # <- 핵심 변경! (vx, vy) 순서 주의
                    ...
                    if line_angle > 90:
                        ...
                        line_angle += 180
                    # tip 좌표로부터 커브 판단
                    signed_tip = (float(tip_x) - x0) * vy - (float(tip_y) - y0) * vx
                    signed_zandi = (float(self.zandi_x) - x0) * vy - (float(self.zandi_y) - y0) * vx 
                ...
                if abs(delta_tip) > self.delta_tip_min:  # 분기 1-1. turn  =  RL 
                    angle = math.degrees(math.atan2(-(tip_y - self.zandi_y), tip_x - self.zandi_x))  # 커브길이면 끝점만 보고 따라가게
                    # 범위를 (-90, 90]로 정규화 (수직에서 좌/우로 기운 각도)
                    if angle > 90:
                        angle -= 180
                    elif angle <= -90:
                ...
                if abs(down_x - up_x) < 2:  # 일직선인 경우
                    ...
                    x1 = x2 = avg_x
                    y1, y2 = self.roi_y_end, self.roi_y_start
                    delta_zandi = float(avg_x - self.zandi_x)
                else:  # 일반적인 경우
                    ...
                    if abs(m) > 1:
                        ...
                        y2 = int(m * x2 + b)
                #----------------------------------------------------------------------------------------------------------#
                if abs(delta_zandi) < self.delta_zandi_min:  # 분기 1-1. out  =  Straight
            ...
            if self.frames_left <= 0: # 15 프레임 지났으면
                ...
                if res != 99:
                    self.get_logger().info(f"[Line] Window done: res= {res}, angle_avg= {mean_angle}, frames= {len(self.status_list)}, "
